...
module.exports = function(app) {
                            ...
                            fs.readFile(manifest, function(err, content) {
                                parser.parseString(content, function(err, parsed) {
                                    try {
                                        ...
                                        try {
                                            widgetProps.package = require( path.join(directory, name, 'package.json') );
                                        } catch (e) {
    ...
    app.controller('WidgetsController', function($scope, $rootScope, WidgetsDir, $q, updateXMLonServer) {
        ...
        $scope.term = localStorage.getItem('term')||'';
        $scope.regex = '';
        $scope.widgets = [];
        $scope.loading = false;
        ...
        $scope.updateXMLonServer = function (widget) {
            updateXMLonServer( path.join( widget.path , 'manifest.xml' ) ).then(function (resp) {
